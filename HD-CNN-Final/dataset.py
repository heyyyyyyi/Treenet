...
def get_all_dataLoders(args, valid=False, one_hot=True):
    ...
    if valid:
        ...
        assert (np.unique(validloader.dataset.train_labels) == np.unique(trainloader.dataset.train_labels)).all()
        if one_hot:
...
def get_dataLoder(args, classes, mode='Train', one_hot=False):
    global trainset, testset, validset
    ...
    if one_hot:
        label_transformer = OneHotEncoder(sparse=False).fit(np.array(required_loader.dataset.train_labels).reshape(-1, 1))  #, categories='auto'
        required_loader.dataset.train_labels = label_transformer.transform(np.array(required_loader.dataset.train_labels).reshape(-1, 1))
