...
class HDEnsemble(object):
    ...
    def init_optimizer(self, num_epochs):
        ...
        self.optimizer = torch.optim.SGD([
            {"params": self.model.share.parameters(), "lr": self.params.lr},  # share: 学得慢一点
            {"params": self.model.subroot_coarse.parameters(), "lr": self.params.lr},  # subroot-coarse: 学得慢一点
    ...
    def forward(self, x, logits=False):
        if not logits:
            coarse_logits, subroot_animal_logits, subroot_vehicle_logits = self.model(x)
        else:
            coarse_logits, subroot_animal_logits, subroot_vehicle_logits = x

        animal_logit_score = torch.logsumexp(coarse_logits[:, animal_classes], dim=1, keepdim=True)
        vehicle_logit_score = torch.logsumexp(coarse_logits[:, vehicle_classes], dim=1, keepdim=True)
        total_logit = torch.logsumexp(torch.cat([animal_logit_score, vehicle_logit_score], dim=1), dim=1, keepdim=True)
        w_animal = torch.exp(animal_logit_score - total_logit)
        w_vehicle = torch.exp(vehicle_logit_score - total_logit)

        animal_classes_index = torch.tensor(animal_classes, device=coarse_logits.device)
        vehicle_classes_index = torch.tensor(vehicle_classes, device=coarse_logits.device)

        animal_logits = torch.full_like(coarse_logits, fill_value=-5.0)
        vehicle_logits = torch.full_like(coarse_logits, fill_value=-5.0)

        animal_logits[:, animal_classes_index] = subroot_animal_logits
        vehicle_logits[:, vehicle_classes_index] = subroot_vehicle_logits

        # shape broadcast
        final_logits = w_animal * animal_logits + w_vehicle * vehicle_logits
        return final_logits


    def fine_tune(self, dataloader, epoch=0, adversarial=False, verbose=True):
        """
    ...
    def standard_loss(self, x, y):
        ...
        loss, subroot_loss_animal, subroot_loss_vehicle, subroot_pt_animal, subroot_pt_vehicle = self.loss_fn(
            [coarse_logits, subroot_animal_logits, subroot_vehicle_logits], y
        )
    ...
    def adversarial_loss(self, x, y):
        ...
        coarse_logits, subroot_animal_logits, subroot_vehicle_logits = self.model(x_adv)
        loss, subroot_loss_animal, subroot_loss_vehicle, subroot_pt_animal, subroot_pt_vehicle = self.loss_fn(
            [coarse_logits, subroot_animal_logits, subroot_vehicle_logits], y_adv
        )
    ...
    def save_share(self, path):
        """Save the shared feature extractor."""
        torch.save(self.model.share.state_dict(), path)

    def save_subroot_coarse(self, path):
        """Save the coarse subroot classifier."""
        torch.save(self.model.subroot_coarse.state_dict(), path)

    def save_subroot_animal(self, path):
        """Save the animal subroot classifier."""
        torch.save(self.model.subroot_animal.state_dict(), path)

    def save_subroot_vehicle(self, path):
        """Save the vehicle subroot classifier."""
    ...
    def load_subroot_coarse(self, path):
        """Load the coarse subroot classifier."""
        self.model.subroot_coarse.load_state_dict(torch.load(path, map_location=device))

    def load_subroot_animal(self, path):
        """Load the animal subroot classifier."""
        self.model.subroot_animal.load_state_dict(torch.load(path, map_location=device))

    def load_subroot_vehicle(self, path):
        """Load the vehicle subroot classifier."""
        self.model.subroot_vehicle.load_state_dict(torch.load(path, map_location=device))
