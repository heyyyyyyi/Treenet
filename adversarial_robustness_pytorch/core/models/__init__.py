...
from .resnet import resnet, lightresnet
...
from .treeresnet import lighttreeresnet
...
MODELS = ['resnet18', 'resnet34', 'resnet50', 'resnet101', 
          ...
          'lighttreeresnet20', 'lightresnet20'
          ]
...
def create_model(name, normalize, info, device):
    ...
    if info['data'] in ['tiny-imagenet']:
        ...
        if 'treeresnet' in name:
            backbone = treeresnet(name, num_classes=info['num_classes'], pretrained=False, device=device)
        elif 'light' in name:
            backbone = light_resnet(name, num_classes=info['num_classes'], pretrained=False, device=device)
